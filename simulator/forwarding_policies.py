...
class LeastResponseTimeNode(Node):
    ...
    def forwardInterest(self, request):
        object_id = request.object_id
        valid_link_avg_delays = np.array([self.link_delays[link].mean for link in self.fib[object_id]])
        rng = np.random.default_rng(1)
        idx = rng.choice(np.where(valid_link_avg_delays == valid_link_avg_delays.min())[0])
