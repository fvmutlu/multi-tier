            ...
            for j, cache in enumerate(self.caches):
                ...
                if object_loc == j:
                    cost_matrix[tier_slice, k] = (
                        (cache.read_rate/cache.capacity) * cs_k + self.pw * cache.read_penalty
                    )
                else:
                    cost_matrix[tier_slice, k] = (
                        (cache.read_rate/cache.capacity) * cs_k - self.pw * cache.write_penalty
                    )
...
class MVIPSBWNode(MVIPNode):
    ...
    def drainVipsByCaching(self):
        for j, cache in enumerate(self.caches):
            for k in self.cacheable_objects:
                self.vip_cache_tx_windows[j, k][-1] = min(
                    self.vip_cache_tx_windows[j, k][-1], self.vip_counts[k]
                )
                self.vip_counts[k] = (
