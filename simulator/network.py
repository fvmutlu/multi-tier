...
class Network(object):
    ...
    def installLinks(self, links, fwd_pol, cache_pol):
        for link in links:
            ...
            ctrl_link_vu = getCtrlLink(
                self.env, fwd_pol, cache_pol, **link["ctrl_args"]
            )
    ...
    def installSources(self, source_nodes, source_read_rate, source_map):
        for node_id in source_nodes:
            self.nodes[node_id].addPermastore(
                Permastore(self.env, source_read_rate, source_map[node_id])
            )
    ...
    def initRequests(self, requester_nodes, requests):
        for node_id in requester_nodes:
            self.env.process(
                self.nodes[node_id].requestGenerator(
                    requests[node_id]["intervals"], requests[node_id]["objects"]
                )
            )

    def terminateSim(self, requester_node_ids):
        node_terminate_events = [
            self.nodes[node_id].can_terminate for node_id in requester_node_ids
        ]
